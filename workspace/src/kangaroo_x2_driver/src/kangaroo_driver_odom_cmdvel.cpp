#include "kangaroo_driver/kangaroo_driver_odom_cmdvel.hpp"
...
bool kangaroo::start( )
{
	...
	if( !joint_traj_sub )
		...
		joint_traj_sub = nh.subscribe( "wheelchair_diff/cmd_vel", 1, &kangaroo::SendVelToKangaroo, this );
	if( !joint_state_pub)
...
// 		ROS_WARN("Joint size: %d, Ch1: %d, Ch2: %d", msg->joint_names.size(), ch1_idx, ch2_idx);
// 		return;
// 	}
...
// 	channel_1_speed = drive_to_encoder_lines(channel_1_speed)*100; // m to cm * real to quadrature
// 	channel_2_speed = turn_to_encoder_lines(channel_2_speed); // real to quadrature * radians to degrees
...
// bool kangaroo::set_channel_units(int desired_units, int machine_units, unsigned char address, char channel) {
// 	if (!is_open() && !open())
// 		return false;

// 	// send
// 	unsigned char buffer[18];
// 	int num_of_bytes = write_kangaroo_units_command(address, channel, desired_units, machine_units, buffer);
...
void kangaroo::JointStateCB( const ros::WallTimerEvent &e )
{
	try
	{
		...
		// if (msg->velocity[0] > 0)
			// ROS_INFO("logger : drive_cb : lines : %f  => drive : %f", msg->velocity[0], msg->velocity[0] * 1005 / 16384);
...
bool kangaroo::send_get_request(unsigned char address, char channel, unsigned char desired_parameter)
{
	...
	// ROS_INFO("%c, %c, %c, %s", address, channel, desired_parameter, buffer);
