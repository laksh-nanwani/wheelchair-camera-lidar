...
def publish_status(broadcaster, status):
    ...
    print("hi")
    broadcaster.sendTransform((status['x']['value'],status['y']['value'],status['z']['value']),
                     tf.transformations.quaternion_from_euler(math.radians(status['roll']['value']),math.radians(status['pitch']['value']),math.radians(status['azimuth']['value'])),
                     rospy.Time.now(),
                     "camera2_link",
                     "camera1_link")
